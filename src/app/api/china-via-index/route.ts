...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Error fetching china VIA index:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await prisma.chinaViaIndex.upsert({
      where: {
        year_month: { year, month },
      },
      update: {
        value,
        unit: (body.unit as string) || "percent",
    ...
    console.error("Error creating china VIA index:", { message: errorMessage, payload: body });
