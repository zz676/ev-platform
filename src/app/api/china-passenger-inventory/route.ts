...
export async function GET(request: NextRequest) {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.chinaPassengerInventory.findMany({
        where,
        orderBy: [{ year: sortOrder as "asc" | "desc" }, { month: sortOrder as "asc" | "desc" }],
        skip,
        take: limit,
      }),
      prisma.chinaPassengerInventory.count({ where }),
    ]);
    ...
    console.error("Error fetching china passenger inventory:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await prisma.chinaPassengerInventory.upsert({
      where: {
        year_month: { year, month },
      },
      update: {
        value,
        unit: (body.unit as string) || "million_units",
    ...
    console.error("Error creating china passenger inventory:", { message: errorMessage, payload: body });
