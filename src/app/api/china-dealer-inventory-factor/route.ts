...
export async function GET(request: NextRequest) {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.chinaDealerInventoryFactor.findMany({
        where,
        orderBy: [{ year: sortOrder as "asc" | "desc" }, { month: sortOrder as "asc" | "desc" }],
        skip,
        take: limit,
      }),
      prisma.chinaDealerInventoryFactor.count({ where }),
    ]);
    ...
    console.error("Error fetching china dealer inventory factor:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await prisma.chinaDealerInventoryFactor.upsert({
      where: {
        year_month: { year, month },
      },
      update: {
        value,
        unit: (body.unit as string) || "ratio",
    ...
    console.error("Error creating china dealer inventory factor:", { message: errorMessage, payload: body });
